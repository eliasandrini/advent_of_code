...
int main() {
  ...
  // keep track of how many removals in the last iteration
  unsigned int last_iteration = 0;

  // iterate until there's no more paper rolls to remove
  do {
    // update counter
    result += last_iteration;
    last_iteration = 0;

    // remove reachable paper rolls
    for (unsigned int row = 0; row < grid.size(); ++row) {
      for (unsigned int col = 0; col < grid[row].size(); ++col) {
        ...
        // if it's reachable remove immediately
        if (counter < MAX_ADJ) {
          last_iteration++;
          grid[row][col] = false;
        }
